...
namespace WorldEdit
{
	public delegate bool Selection(int i, int j, ServerPlayer player);
	...
	public class WorldEdit : TerrariaPlugin
	{
		...
		private void OnGetData(GetDataEventArgs e)
		{
			...
			switch (e.MsgID)
			{
				...
				case PacketTypes.Tile:
					PlayerInfo info = ServerBase.Players[e.Msg.whoAmI].GetPlayerInfo();
					if (info.Point != 0)
					{
						using (var reader = new BinaryReader(new MemoryStream(e.Msg.readBuffer, e.Index, e.Length)))
						{
							...
							if (x >= 0 && y >= 0 && x < Main.maxTilesX && y < Main.maxTilesY)
							{
								if (info.Point == 1)
								{
									info.X = x;
									info.Y = y;
									ServerBase.Players[e.Msg.whoAmI].SendInfoMessage("Set point 1.");
								}
								else if (info.Point == 2)
								{
									info.X2 = x;
									info.Y2 = y;
									ServerBase.Players[e.Msg.whoAmI].SendInfoMessage("Set point 2.");
								}
								else if (info.Point == 3)
								{
									List<string> regions = RegionManager.GetRegionsInArea(x, y).Select(b=>b.Name).ToList();
                                    if (regions.Count == 0)
                                    {
	                                    ServerBase.Players[e.Msg.whoAmI].SendErrorMessage("No region exists there.");
                                    }
                                    else
                                    {
                                        Region curReg = RegionManager.GetRegionByName(regions[0]);
                                        info.X = curReg.Area.Left;
                                        info.Y = curReg.Area.Top;
                                        info.X2 = curReg.Area.Right;
                                        info.Y2 = curReg.Area.Bottom;
                                        ServerBase.Players[e.Msg.whoAmI].SendInfoMessage("Set region.");
                                    }
								}
                                else if (info.Point == 4)
                                {
                                    if (!MagicWand.GetMagicWandSelection(x, y,
                                        ...
                                        ServerBase.Players[e.Msg.whoAmI], out MagicWand selection))
                                    {
	                                    ServerBase.Players[e.Msg.whoAmI].SendErrorMessage("Can't " +
                                            "start counting magic wand selection from this tile.");
                                    }
                                    else
                                    {
                                        info.MagicWand = selection;
                                        ServerBase.Players[e.Msg.whoAmI].SendSuccessMessage("Set magic wand selection.");
                                    }
                                    info.SavedExpression = null;
                                }
                                info.Point = 0;
								e.Handled = true;
								ServerBase.Players[e.Msg.whoAmI].SendTileSquare(x, y, 3);
							}
						}
					}
					return;
				...
				case PacketTypes.MassWireOperation:
					PlayerInfo data = ServerBase.Players[e.Msg.whoAmI].GetPlayerInfo();
					if (data.Point != 0)
					{
						using (var reader = new BinaryReader(new MemoryStream(e.Msg.readBuffer, e.Index, e.Length)))
						{
							...
							if (startX >= 0 && startY >= 0 && endX >= 0 && endY >= 0 && startX < Main.maxTilesX && startY < Main.maxTilesY && endX < Main.maxTilesX && endY < Main.maxTilesY)
                            {
                                ...
                                else if (startX == endX && startY == endY)
								{
									...
									{
										List<string> regions = RegionManager.GetRegionsInArea(startX, startY).Select(x=>x.Name).ToList();
										if (regions.Count == 0)
										{
											ServerBase.Players[e.Msg.whoAmI].SendErrorMessage("No region exists there.");
										}
								...
								else
								{
									...
									if (data.Point == 1 || data.Point == 2)
									{
										...
										data.X2 = endX;
										data.Y2 = endY;
										ServerBase.Players[e.Msg.whoAmI].SendInfoMessage("Set area.");
									}
									else if (data.Point == 3)
									{
										...
										List<Region> regions = RegionManager.Regions.FindAll(r => rect.Intersects(r.Area));
										if (regions.Count == 0)
										{
											ServerBase.Players[e.Msg.whoAmI].SendErrorMessage("No region exists there.");
										}
										...
										{
											Region curReg = RegionManager.GetTopRegion(regions);
											data.X = curReg.Area.Left;
											data.Y = curReg.Area.Top;
											data.X2 = curReg.Area.Right;
											data.Y2 = curReg.Area.Bottom;
		...
		private void OnInitialize(EventArgs e)
		{
            ...
            #region Commands
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.admin", EditConfig, "/worldedit", "/wedit")
            {
                HelpText = "Edits config options."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.activate", Activate, "/activate")
			{
				HelpText = "Activates non-working signs, chests or item frames."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.all", All, "/all")
			{
				HelpText = "Sets the worldedit selection to the entire world."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.biome", Biome, "/biome")
			{
				HelpText = "Converts biomes in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.clipboard.copy", Copy, "/copy", "/c")
			{
				HelpText = "Copies the worldedit selection to the clipboard."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.clipboard.cut", Cut, "/cut")
			{
				HelpText = "Copies the worldedit selection to the clipboard, then deletes it."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.drain", Drain, "/drain")
			{
				HelpText = "Drains liquids in the worldedit selection."
			});
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.fill", Fill, "/fill")
            {
                HelpText = "Fills the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.fillwall", FillWall, "/fillwall", "/fillw")
            {
                HelpText = "Fills the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.fixghosts", FixGhosts, "/fixghosts")
			{
				HelpText = "Fixes invisible signs, chests and item frames."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.fixgrass", FixGrass, "/fixgrass")
			{
				HelpText = "Fixes suffocated grass in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.fixhalves", FixHalves, "/fixhalves")
			{
				HelpText = "Fixes half blocks in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.fixslopes", FixSlopes, "/fixslopes")
			{
				HelpText = "Fixes covered slopes in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.clipboard.flip", Flip, "/flip")
			{
				HelpText = "Flips the worldedit clipboard."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.flood", Flood, "/flood")
			{
				HelpText = "Floods liquids in the worldedit selection."
			});
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.magic.wand", MagicWandTool, "/magicwand", "/mwand")
            {
                HelpText = "Creates worldedit selection from contiguous tiles that are matching boolean expression."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.killempty", KillEmpty, "/killempty")
            {
                HelpText = "Deletes empty signs and/or chests (only entities, doesn't remove tiles)."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.move", Move, "/move")
            {
                HelpText = "Moves tiles from the worldedit selection to new area."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.mow", Mow, "/mow")
			{
				HelpText = "Mows grass, thorns, and vines in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.near", Near, "/near")
			{
				AllowServer = false,
				HelpText = "Sets the worldedit selection to a radius around you."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.outline", Outline, "/outline", "/ol")
			{
				HelpText = "Sets block outline around blocks in area."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.outlinewall", OutlineWall, "/outlinewall", "/olw")
			{
				HelpText = "Sets wall outline around walls in area."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.paint", Paint, "/paint", "/pa")
			{
				HelpText = "Paints tiles in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.paintwall", PaintWall, "/paintwall", "/paw")
			{
				HelpText = "Paints walls in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.clipboard.paste", Paste, "/paste", "/p")
			{
				HelpText = "Pastes the clipboard to the worldedit selection."
			});
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.clipboard.spaste", SPaste, "/spaste", "/sp")
            {
                HelpText = "Pastes the clipboard to the worldedit selection with certain conditions."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.point", Point1, "/point1", "/p1", "p1")
			{
				HelpText = "Sets the positions of the worldedit selection's first point."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.point", Point2, "/point2", "/p2", "p2")
			{
				HelpText = "Sets the positions of the worldedit selection's second point."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.history.redo", Redo, "/redo")
			{
				HelpText = "Redoes a number of worldedit actions."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.region", RegionCmd, "/region")
			{
				HelpText = "Selects a region as a worldedit selection."
			});
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.replace", Replace, "/replace", "/rep")
            {
                HelpText = "Replaces tiles in the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.replacewall", ReplaceWall, "/replacewall", "/repw")
            {
                HelpText = "Replaces walls in the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.resize", Resize, "/resize")
			{
				HelpText = "Resizes the worldedit selection in a direction."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.clipboard.rotate", Rotate, "/rotate")
			{
				HelpText = "Rotates the worldedit clipboard."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.schematic", Schematic, "/schematic", "/schem", "/sc", "sc")
			{
				HelpText = "Manages worldedit schematics."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.selecttype", Select, "/select")
			{
				HelpText = "Sets the worldedit selection function."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.set", Set, "/set")
			{
				HelpText = "Sets tiles in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.setgrass", SetGrass, "/setgrass")
			{
				HelpText = "Sets certain grass in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.setwall", SetWall, "/setwall", "/swa")
			{
				HelpText = "Sets walls in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.setwire", SetWire, "/setwire", "/swi")
			{
				HelpText = "Sets wires in the worldedit selection."
			});
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.shape", Shape, "/shape")
            {
                HelpText = "Draws line/rectangle/ellipse/triangle in the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.shape", Shape, "/shapefill", "/shapef")
            {
                HelpText = "Draws line/rectangle/ellipse/triangle in the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.shape", Shape, "/shapewall", "/shapew")
            {
                HelpText = "Draws line/rectangle/ellipse/triangle in the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.shape", Shape, "/shapewallfill", "/shapewf")
            {
                HelpText = "Draws line/rectangle/ellipse/triangle in the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.utils.size", Size, "/size")
            {
                HelpText = "Shows size of clipboard or schematic."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.slope", Slope, "/slope")
			{
				HelpText = "Slopes tiles in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.delslope", SlopeDelete, "/delslope", "/delslopes", "/dslope", "/dslopes")
			{
				HelpText = "Removes slopes in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.smooth", Smooth, "/smooth")
			{
				HelpText = "Smooths blocks in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.inactive", Inactive, "/inactive", "/ia")
			{
				HelpText = "Sets the inactive status in the worldedit selection."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.shift", Shift, "/shift")
			{
				HelpText = "Shifts the worldedit selection in a direction."
			});
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.selection.text", Text, "/text")
            {
                HelpText = "Creates text with alphabet statues in the worldedit selection."
            });
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.history.undo", Undo, "/undo")
			{
				HelpText = "Undoes a number of worldedit actions."
			});
			TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.clipboard.scale", Scale, "/scale")
			{
				HelpText = "Scale the clipboard"
			});
            TShockAPI.Commands.ServerCommands.Add(new Command("worldedit.region.actuator", Actuator, "/actuator")
            {
            ...
            {
	            Database = ServerBase.LocalDatabase;
            }
            else
            {
	            Database = ServerBase.GlobalDatabase;
            }
			...
			if (!File.Exists(lockFilePath))
			{
				...
				File.Create(lockFilePath).Close();
				ServerBase.Log.ConsoleInfo("WorldEdit doesn't support undo/redo/clipboard files that were saved by plugin below version 1.7.");
				ServerBase.Log.ConsoleInfo("These files had been deleted. However, we still support old schematic files (*.dat)");
				ServerBase.Log.ConsoleInfo("Do not delete deteted.lock inside worldedit folder; this message will only show once.");
			}
			#endregion
		...
		private void Flip(CommandArgs e)
        {
			...
			else if (!Tools.HasClipboard(e.Player.Account.AccountId))
		...
		private void Paste(CommandArgs e)
        {
			...
			else
			{
                ...
                _commandQueue.Add(new Paste(info.X, info.Y, e.Player, Tools.GetClipboardPath(e.Player.Account.AccountId), alignment, expression, mode_MainBlocks, true));
			}
        }
		...
		private void Redo(CommandArgs e)
        {
			...
			int steps = 1;
			int ID = e.Player.Account.AccountId;
			...
			else
			{
				if (e.Parameters.Count > 1)
                {
                    ...
                    UserAccount Account = UserAccountManager.GetUserAccountByName(e.Parameters[1]);
					if (Account == null)
					{
			...
			_commandQueue.Add(new Redo(e.Player, steps));
		}
		...
		private void RegionCmd(CommandArgs e)
		{
			...
			{
				Region region = RegionManager.GetRegionByName(e.Parameters[0]);
				if (region == null)
		...
		private void Rotate(CommandArgs e)
        {
			...
			if (!Tools.HasClipboard(e.Player.Account.AccountId))
			{
				e.Player.SendErrorMessage("Invalid clipboard!");
				return;
			}
				...
				case "load":
                    {
						...
						var clipboard = Tools.GetClipboardPath(e.Player.Account.AccountId);
				...
				case "save":
                    {
                            ...
                            && e.Parameters.ElementAtOrDefault(1)?.ToLower() == "id")
                        {
                            ...
                            UserAccount account = UserAccountManager.GetUserAccountByName(uname);
                            if (account == null)
                            {
                            ...
                            e.Player.SendSuccessMessage($"{account.Name}'s ID: {account.AccountId}.");
                            return;
                        }
						...
						string clipboard = Tools.GetClipboardPath(e.Player.Account.AccountId);
                        ...
                        if (Config.StartSchematicNamesWithCreatorUserID)
                            name = $"{e.Player.Account.AccountId}-{name}";
                ...
                case "clipboard":
                    {
                        ...
                        if (e.Parameters.Count > 1)
                        {
                            ...
                            account = UserAccountManager.GetUserAccountByName(e.Parameters[1]);
                            if (account == null)
                            {
                                e.Player.SendErrorMessage("Invalid account name!");
                                return;
                        ...
                        if (!Tools.HasClipboard(account.AccountId))
                        {
                            e.Player.SendErrorMessage($"{account.Name} doesn't have a clipboard.");
                            return;
                        }

                        WorldSectionData data = Tools.LoadWorldData(Tools.GetClipboardPath(account.AccountId));
		...
		private void Undo(CommandArgs e)
        {
			...
			else if (e.Parameters.Count > 1)
            {
                ...
                UserAccount User = UserAccountManager.GetUserAccountByName(e.Parameters[1]);
				if (User == null)
				{
					e.Player.SendErrorMessage("Invalid account name!");
					return;
			...
			_commandQueue.Add(new Undo(e.Player, steps));
		}
	}
}
