...
namespace WorldEdit
{
    public static class Tools
    {
        ...
        public static bool Translate(string path, bool logError, string tempCopyPath = null)
        {
            ...
            {
                if (logError)
                    ServerBase.Log.ConsoleError($"[WorldEdit] File '{path}' could not be converted to Terraria v1.4:\n{e}");
                translated = false;
            }
		...
		public static void PrepareUndo(int x, int y, int x2, int y2, ServerPlayer plr)
		{
            if (WorldEdit.Config.DisableUndoSystemForUnrealPlayers && !plr.RealPlayer)
                return;

            var dbData = DB.DB.InsertIfNotExists(plr);

            var undoLevel = dbData.UndoLevel + 1;
            DB.DB.SetRedoLevel(plr, -1);
            DB.DB.SetUndoLevel(plr, undoLevel);

			string path = Path.Combine("worldedit", string.Format("undo-{0}-{1}-{2}.dat", Main.worldID, plr.Account.AccountId, undoLevel));
			...
			File.Delete(Path.Combine("worldedit", string.Format("undo-{0}-{1}-{2}.dat", Main.worldID, plr.Account.AccountId, undoLevel - MAX_UNDOS)));
		}

		public static bool Redo(ServerPlayer plr)
        {
            if (WorldEdit.Config.DisableUndoSystemForUnrealPlayers && plr.Account.AccountId == 0)
                return false;

            var dbData = DB.DB.InsertIfNotExists(plr);
            
            int redoLevel = dbData.RedoLevel - 1;
			int undoLevel = dbData.UndoLevel + 1;

			if (redoLevel < -1)
				return false;

			string redoPath = Path.Combine("worldedit", string.Format("redo-{0}-{1}-{2}.dat", Main.worldID, plr.Account.AccountId, redoLevel + 1));
            DB.DB.SetRedoLevel(plr, redoLevel);
		...
		public static bool Undo(ServerPlayer plr)
        {
            if (WorldEdit.Config.DisableUndoSystemForUnrealPlayers && plr.Account.AccountId == 0)
                return false;
			...
			string undoPath = Path.Combine("worldedit", string.Format("undo-{0}-{1}-{2}.dat", Main.worldID, plr.Account.AccountId, undoLevel + 1));
            DB.DB.SetUndoLevel(plr,undoLevel);

			if (!File.Exists(undoPath))
				return false;

			string redoPath = Path.Combine("worldedit", string.Format("redo-{0}-{1}-{2}.dat", Main.worldID, plr.Account.AccountId, redoLevel));
            DB.DB.SetRedoLevel(plr,redoLevel);
        ...
        public static bool CanSet(bool Tile, ITile tile, int type,
            ...
            MagicWand magicWand, int x, int y, ServerPlayer player) =>
            Tile
