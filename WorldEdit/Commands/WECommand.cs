...
namespace WorldEdit.Commands
{
	public abstract class WECommand
	{
        ...
        protected WECommand(int x, int y, int x2, int y2, ServerPlayer plr)
            : this(x, y, x2, y2, null, plr) { }

        protected WECommand(int x, int y, int x2, int y2, MagicWand magicWand,
            ServerPlayer plr, bool minMaxPoints = true)
		{
			this.plr = plr;
        ...
        public bool CanUseCommand(int x, int y, int x2, int y2)
        {
            ...
            if ((!noRegion && !RegionManager.Regions.Any(r => r.Area.Contains(area)))
                || !RegionManager.CanBuild(x, y, plr)
                || !RegionManager.CanBuild(x2, y, plr)
                || !RegionManager.CanBuild(x2, y2, plr)
                || !RegionManager.CanBuild(x, y2, plr))
            {
